...
- **Color Splash graphic-pack workarounds bundled**: shipped the community "Invisible Characters in Cutout Mode" and "Green Characters Tint" packs (from [cemu-project/cemu_graphic_packs](https://github.com/cemu-project/cemu_graphic_packs), credit to RedHalted) so they appear in the in-app Graphic Packs UI by default and just need to be toggled on. Fixes the horizontal black bars on characters during the paper-wave animation.
